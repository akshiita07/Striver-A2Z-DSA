...
class Solution {
public:
    long long dfs(int node, int parent, const vector<vector<int>>& tree,
        ...
        for (auto it : tree[node]) {
            if (it != parent) {
                // Skip the parent to avoid revisiting
                sum += dfs(it, node, tree, values, k,
                           ans); // Add the value of the subtree
            }
        }
    ...
    int maxKDivisibleComponents(int n, vector<vector<int>>& edges,
        ...
        // edges[i] = [ai, bi] : edge between nodes ai and bi in the tree
        // values[i] is the value associated with the ith node, and an integer k
        // valid split of tree: resulting components all have values that are
        // divisible by k value of a connected component is the sum of the
        // values of its nodes. Return the maximum number of components in any
        // valid split Build the tree using an adjacency list
        vector<vector<int>> tree(n);
        for (const auto& edge : edges) {
            tree[edge[0]].push_back(edge[1]);
            tree[edge[1]].push_back(edge[0]);
