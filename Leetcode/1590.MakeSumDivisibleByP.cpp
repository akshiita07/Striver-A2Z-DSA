...
public:
    int minSubarray(vector<int>& nums, int p) {
        ...
        // If the total sum is already divisible by p, no need to remove any
        // subarray
        if (totalSum == 0) {
            return 0;
        }
        ...
        for (int i = 0; i < nums.size(); i++) {
            ...
            // If target remainder is found in the map, calculate the subarray
            // length
            if (prefixSumRemainder.find(target) != prefixSumRemainder.end()) {
                minLength = min(minLength, i - prefixSumRemainder[target]);
            }
        ...
        // If the minimum length found is equal to the size of the array, return
        // -1
        return minLength == nums.size() ? -1 : minLength;
    }
};
