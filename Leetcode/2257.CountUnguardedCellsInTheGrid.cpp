...
int countUnguarded(int m, int n, vector<vector<int>> &guards, vector<vector<int>> &walls)
{
    ...
    // guard can see every cell in the four cardinal directions unless obstructed by a wall or another guard.
    ...
    for (auto &it : guards)
    {
        ...
        for (const auto &dir : directions)
        {
            ...
            int nx = x + dx;
            int ny = y + dy;
            while (nx >= 0 && nx < m && ny >= 0 && ny < n && createGrid[nx][ny] != 1 && createGrid[nx][ny] != 2) // within coordinates & not a guard or wall then under visiblility
            {
                if (createGrid[nx][ny] == 0)
                {
                    createGrid[nx][ny] = 3; // Mark as guarded as specifed  by 3
                }
                nx += dx;
                ny += dy;
            }
...
int main()
{
    ...
    // Output: 7
    cout << "\nNumber of unoccupied cells are: " << countUnguarded(m, n, guards, walls);
